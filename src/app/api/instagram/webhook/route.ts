...
export async function POST(request: Request) {
  ...
  for (const entry of entries) {
    ...
    for (const event of messaging) {
      ...
      const igId = sender?.id?.toString()
      if (!igId) continue
      const text = message.text as string
      const externalMessageId = message.mid?.toString() ?? crypto.randomUUID()
      const result = await processInboundText(
        db as any, accountId, 'instagram', igId, text, externalMessageId,
      )
      if (!result.success) {
        console.error(`[instagram-webhook] processInboundText error:`, result.error)
      }
    }
  }
